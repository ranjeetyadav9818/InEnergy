...
@Named
public class LazyNotificationDataModel extends LazyDataModel<PdpSrNotification> {
	...
	Logger log =  LoggerFactory.getLogger(LazyNotificationDataModel.class);
	
	private PdpSrEvent event;
	private List<PdpSrNotification> datasource = new ArrayList<PdpSrNotification>();
	private NotificationType type = NotificationType.ALL; 
	private EventList eventList;
	private String filterString = null;
	private String enpValue = null;
	private PdpSrParticipant participant;
	...
	public LazyNotificationDataModel(EventList eventList, PdpSrEvent event){
		this.event = event;
		this.eventList = eventList;
	}
	
	public LazyNotificationDataModel(EventList eventList, PdpSrEvent event, NotificationType type){
		this.event = event;
		this.type = type;
		this.eventList = eventList;
	}
	
	public LazyNotificationDataModel(EventList eventList, PdpSrEvent event, NotificationType type, String enpValue){
		this.event = event;
		this.type = type;
		this.eventList = eventList;
	...
	public LazyNotificationDataModel(EventList eventList, PdpSrEvent event, NotificationType type, PdpSrParticipant participant){
		this.event = event;
		this.type = type;
		this.eventList = eventList;
	...
	@Override
	public PdpSrNotification getRowData(String rowKey){
		Long l = Long.valueOf(rowKey);
		for(PdpSrNotification notification : datasource){
	...
	@Override
	public Object getRowKey(PdpSrNotification notification){
		return notification.getNotificationId();
	}
	
	@Override
	public List<PdpSrNotification> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String,Object> filters) {
		this.participantJoined = false;
		
		if(eventList != null){
			if(this.event!=null){
				Session session = (Session)eventList.getEntityManager().getDelegate();
				Criteria searchCriteria = session.createCriteria(PdpSrNotification.class);
				...
				if(filterString == null || !filterString.equals(stringBuilderForFilter.toString())){
					Criteria countSearchCriteria = session.createCriteria(PdpSrNotification.class);
...
//			new Exception().printStackTrace();
	...
	public List<PdpSrNotification> getDatasource() {
		return datasource;
	}

	public void setDatasource(List<PdpSrNotification> datasource) {
