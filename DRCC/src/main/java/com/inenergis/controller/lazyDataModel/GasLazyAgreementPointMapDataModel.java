...
import com.inenergis.controller.servicePointData.GasServicePointList;
...
@Transactional
public class GasLazyAgreementPointMapDataModel extends LazyDataModel<AgreementPointMap> {
    ...
    Logger log = LoggerFactory.getLogger(GasLazyAgreementPointMapDataModel.class);
    ...
    private GasServicePointList servicePointList;

    public GasLazyAgreementPointMapDataModel(GasAgreementPointMapList agreementPointMapList) {
        this.agreementPointMapList = agreementPointMapList;
    }

    public GasLazyAgreementPointMapDataModel(String personId, String saId, String name, String address, String city, String postCode, String premisesId, String accountId, String phone, GasAgreementPointMapList agreementPointMapList) {
    ...
    public GasLazyAgreementPointMapDataModel(String address, String city, String postCode, String servicePointId, String premisesId, GasServicePointList servicePointList) {
        this.personId = null;
    ...
    public List<AgreementPointMap> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
        ...
        if (filters != null && filters.containsKey(PHONE)) {
            filters.replace(PHONE, GasServiceAgreement.stripOutNoDigit((String) filters.get(PHONE)));
        }
    ...
    private void populateFilterCriteria(Map<String, Object> filters, Criteria c) {
        ...
        if (StringUtils.isNotBlank(GasServiceAgreement.stripOutNoDigit(this.phone))) {
            c.add(like("sa.phone", GasServiceAgreement.stripOutNoDigit(this.phone), MatchMode.START));
        }
        ...
        if (filters != null && !filters.isEmpty()) {
            ...
            if (filters.containsKey("stat")) {
                c.add(like("sa.saStatus", ((GasServiceAgreement.SaStatus) filters.get("stat")).getValue(), MatchMode.EXACT));
            }

            if (filters.containsKey("meterId")) {
                c.add(like("mtr1.meterId", ((String) filters.get("meterId")).trim(), MatchMode.START));
            }

            if (filters.containsKey("meterBadgeNumber")) {
                c.add(like("mtr1.badgeNumber", ((String) filters.get("meterBadgeNumber")).trim(), MatchMode.START));
            }
        }
    }
